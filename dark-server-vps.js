...
const https = require('https'); // 使用HTTPS代替HTTP
...
class ProxyServerSystem extends EventEmitter {
  constructor(config = {}) {
    super();
    this.config = {
      port: 8889, // HTTPS 和 WSS 使用的统一端口
      host: '0.0.0.0',
      // SSL证书文件路径。您需要生成这些文件。
      // 例如, 使用 OpenSSL: openssl req -x509 -newkey rsa:2048 -keyout key.pem -out cert.pem -sha256 -days 365 -nodes
      sslKeyPath: './key.pem',
    ...
    this.server = null; // 用于存储HTTPS服务器实例
    this.wsServer = null;
  }
  
  async start() {
    try {
      await this._startServer(); // 启动统一的服务器
  ...
  async _startServer() {
    ...
    try {
      ...
      this.logger.error(`无法读取SSL证书文件: ${err.message}`);
      this.logger.error(`请确保 '${this.config.sslKeyPath}' 和 '${this.config.sslCertPath}' 文件存在。`);
      this.logger.error('您可以使用 OpenSSL 生成自签名证书:');
      this.logger.error('openssl req -x509 -newkey rsa:2048 -keyout key.pem -out cert.pem -sha256 -days 365 -nodes');
      throw new Error('SSL证书配置错误');
    }
    ...
    return new Promise((resolve) => {
      this.server.listen(this.config.port, this.config.host, () => {
        this.logger.info(`HTTPS服务器启动: https://${this.config.host}:${this.config.port}`);
        this.logger.info(`WSS服务器正在监听: wss://${this.config.host}:${this.config.port}`);
        resolve();
      });
    });
  }
