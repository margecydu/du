...
class ConnectionManager extends EventTarget {
  ...
  constructor(endpoint = 'wss://YOUR_SERVER_IP_OR_DOMAIN:8889') {
    // 请将 YOUR_SERVER_IP_OR_DOMAIN 替换为您的服务器公网IP或域名
    // 端口号应与服务器的 httpPort (现在是https端口) 匹配
    super();
  ...
  // ... [ a large portion of ConnectionManager, RequestProcessor, StreamHandler, and ProxySystem classes are UNCHANGED ] ...
  // ... [ 为了简洁，我省略了未改变的代码，请保留您文件中的这些部分 ] ...
  async establish() {
    if (this.isConnected) {
...
// [ RequestProcessor class - UNCHANGED ]
...
async function initializeProxySystem() {
  const proxySystem = new ProxySystem(); // This will now use the new default wss:// endpoint
